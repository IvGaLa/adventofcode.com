...
const parseRow = (rows) =>
  rows[0]
    .split('')
    .map((_, x) =>
      rows.reduce((count, row) => count + (row[x] === '#' ? 1 : 0), 0),
    )
    .join('');

const parseInput = (fileInput) => {
  const data = _readInput(fileInput);
  const inputs = { keys: [], locks: [] };

  for (let i = 0; i < data.length; i += 7) {
    const index = data[i] === TOP && data[i + 6] === BOTTOM ? 'locks' : 'keys';
    const rows = data.slice(i + 1, i + 6);
    inputs[index].push(parseRow(rows));
  }

  return inputs;
};
const day25 = (keys, locks) => {
  let count = 0;
  ...
  for (const lock of locks)
    for (const key of keys) {
      ...
      if (l.every((chrLock, i) => chrLock + k[i] <= 5)) count++;
    }
...
// const day25Two = (fileInput) => {
//   const data = _readInput(fileInput);

//   return data;
// };

//const fileInput = './2024/day25/example.txt'; // 3 -
const fileInput = './2024/day25/input.txt'; // 3284 -

const { keys, locks } = parseInput(fileInput);

console.log(day25(keys, locks));
// console.log(day25Two(fileInput));
