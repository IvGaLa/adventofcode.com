...
const parseInput = (fileInput) => {
  return _readInput(fileInput).reduce(
    (acc, line) => {
      if (line.includes(':')) {
        const c = line.split(': ');
        acc.cables[c[0]] = Number(c[1]);
      } else acc.gates.push(line.split(' ').filter((_, i) => i !== 3));
      return acc;
    },
    { cables: {}, gates: [] },
  );
...
const day24 = (fileInput) => {
  let { cables, gates } = parseInput(fileInput);

  while (gates.length) {
    const nextOperations = [];
    for (const [cableAKey, gateKey, cableBKey, resultKey] of gates) {
      if (cableAKey in cables && cableBKey in cables) {
        cables[resultKey] = ops[gateKey](cables[cableAKey], cables[cableBKey]);
        continue;
      }
      nextOperations.push([cableAKey, gateKey, cableBKey, resultKey]);
    }
    gates = nextOperations;
  }
...
const findGate = (cableA, gate, cableB, gates) => {
  const [cA, _, cB, result] = gates.find(
    ([cA, g, cB]) =>
      g === gate &&
      (cA === cableA || cA === cableB || cB === cableA || cB === cableB),
  );

  if (cA !== cableA && cA !== cableB) return [result, cA];
  if (cB !== cableA && cB !== cableB) return [result, cB];
  return [result];
};

// --------------------------------------------------------

const day24Two = (fileInput) => {
  const { gates } = parseInput(fileInput);
  const badGates = new Set();
  let [carry] = findGate(`x00`, 'AND', `y00`, gates);
  let i = 2;

  while (i < 45) {
    const [currentXOR] = findGate(`x${pad(i)}`, 'XOR', `y${pad(i)}`, gates);
    const [lastAND] = findGate(
      `x${pad(i - 1)}`,
      'AND',
      `y${pad(i - 1)}`,
      gates,
    );
    const [lastXOR] = findGate(
      `x${pad(i - 1)}`,
      'XOR',
      `y${pad(i - 1)}`,
      gates,
    );
    const [cable3, bad1] = findGate(carry, 'AND', lastXOR, gates);
    const [cable2, bad2] = findGate(lastAND, 'OR', cable3, gates);
    const [result, bad3] = findGate(currentXOR, 'XOR', cable2, gates);
    const gateZ = `z${pad(i)}`;

    [bad1, bad2, bad3].forEach((wrongGate) => {
      if (wrongGate) badGates.add(wrongGate);
    });
    if (result !== gateZ) badGates.add(gateZ);

    carry = cable2;
    i++;
  }
  return [...badGates].sort().join(',');
};

//const fileInput = './2024/day24/example.txt'; // 2024
const fileInput = './2024/day24/input.txt'; // 45213383376616 - cnk,mps,msq,qwf,vhm,z14,z27,z39

console.log(day24(fileInput));
console.log(day24Two(fileInput));
