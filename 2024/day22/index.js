...
const parseInput = (fileInput) => _readInput(fileInput).map(Number);
...
const getSecrets = (secret) => {
  ...
  for (let i = 0; i < LIMIT; i++) {
    ...
    secrets.push(secret);
    lastDigits.push(secret % 10);
  }

  return [secrets, lastDigits];
};

const day22 = (fileInput) =>
  parseInput(fileInput).reduce(
    (sum, secret) => sum + getSecrets(secret)[0].at(-1),
    0,
  );

// --------------------------------------------------------

const day22Two = (fileInput) => {
  const count = new Map();

  parseInput(fileInput)
    .map((secret) => getSecrets(secret)[1])
    .forEach((digits) => {
      const look = new Set();
      let [d1, d2, d3, d4, d5] = [
        digits[0],
      ...
      for (let i = 0; i < digits.length - 4; i++) {
        if (i > 0) {
          ...
          d2 = d3;
          d3 = d4;
          d4 = digits[i + 4] - digits[i + 3];
        }
        const keymap = `${d1}-${d2}-${d3}-${d4}`;
        if (!look.has(keymap)) {
          look.add(keymap);
          count.set(keymap, (count.get(keymap) || 0) + digits[i + 4]);
        }
      }
    });
...
//const fileInput = './2024/day22/example.txt'; // 37327623
//const fileInput = './2024/day22/example2.txt'; // 23
const fileInput = './2024/day22/input.txt'; // 13584398738 - 1612

console.log(day22(fileInput));
console.log(day22Two(fileInput));
