...
const bfs = (startX, startY, plantType) => {
  let area = 0;
  let perimeter = 0;
  ...
  while (queue.length > 0) {
    ...
    for (const [dx, dy] of directions) {
      ...
      if (!isValid(nx, ny, map) || map[nx][ny] !== plantType) {
        perimeter++;
...
const calculateRegionMetrics = () => {
  ...
  for (let i = 0; i < map.length; i++) {
    for (let j = 0; j < map[0].length; j++) {
      if (!visited.has(`${i},${j}`)) {
        const plantType = map[i][j];
        const { area, perimeter } = bfs(i, j, plantType);
...
const day12 = () => {
  const regions = calculateRegionMetrics(map);

  return regions.reduce((sum, { area, perimeter }) => {
