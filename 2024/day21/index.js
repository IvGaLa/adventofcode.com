...
const WALLLINE = WALL.repeat(5).split('');
...
const DIRS_LIT = [UP, RIGHT, DOWN, LEFT];
...
const DIRPAD = [
  WALLLINE,
  [WALL, WALL, UP, A, WALL],
  [WALL, LEFT, DOWN, RIGHT, WALL],
  WALLLINE,
];

const parseInput = (fileInput) => _readInput(fileInput);

const findingPaths = (map, startVal, endVal) => {
  let start = [];

  map.forEach((row, y) =>
    row.forEach((value, x) => {
      if (value === startVal) start = [x, y];
    }),
  );
  ...
  while ((current = stack.shift())) {
    if (current.dirId !== undefined) current.path.push(DIRS_LIT[current.dirId]);

    if (map[current.position[1]][current.position[0]] === endVal) {
    ...
    let key = current.position.join(SEPARATOR);

    if (seen[key] < current.cost) continue;
    seen[key] = current.cost;
    if (current.cost > minCost) continue;

    DIRECTIONS.forEach((direction, dirId) => {
      let position = [
        current.position[0] + direction[0],
        current.position[1] + direction[1],
      ];
      if (map[position[1]][position[0]] === WALL) return true;
      stack.push({
  ...
  return paths.map((path) => path.join('') + A);
};

const checkPaths = (map, code, depth, memo = {}) => {
  let index = code + SEPARATOR + depth;
  ...
  code.split('').forEach((nextPos) => {
    let paths = findingPaths(map, curPos, nextPos);
    length +=
      depth === 0
        ? paths[0].length
        : Math.min(
            ...paths.map((path) => checkPaths(DIRPAD, path, depth - 1, memo)),
          );
    curPos = nextPos;
  });
...
const day21 = (fileInput) => {
  const codes = parseInput(fileInput);
  const robots = 2;
  const count = codes.reduce(
    (res, code) => res + parseInt(code) * checkPaths(NUMPAD, code, robots),
    0,
  );
...
//const fileInput = './2024/day21/example.txt'; // 126384 - 154115708116294
const fileInput = './2024/day21/input.txt'; // 182844 - 226179529377982

console.log(day21(fileInput));
console.log(day21Two(fileInput));
