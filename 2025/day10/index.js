...
const getMachines = (fileInput) => {
  ...
  for (const machine of data) {
    ...
    requirements.push([...parts[0].slice(1, -1)].map((c) => c === '#'));

    buttons.push(
      parts.slice(1, -1).map((b) => b.slice(1, -1).split(',').map(Number)),
    );

    joltage.push(parts.at(-1).slice(1, -1).split(',').map(Number));
  }

  return [requirements, buttons, joltage];
};

const parseButtons = (requirement, buttons, lights, index, depth) => {
  let minimum = 0;

  for (let i = index; i < buttons.length; i++) {
    const reverseLights = lights.slice();

    for (const light of buttons[i])
      reverseLights[light] = !reverseLights[light];

    if (reverseLights.every((x, j) => x === requirement[j])) return depth;
    ...
    if (result > 0 && (minimum === 0 || result < minimum)) minimum = result;
  }

  return minimum;
};

const day10 = ([requirements, wirings]) => {
  let count = 0;
  ...
  for (let i = 0; i < requirements.length; i++) {
    count += parseButtons(
      requirements[i],
      wirings[i],
      Array(requirements[i].length).fill(false),
      0,
      1,
    );
  }
...
// --------------------------------------------------------
/**
I was unable to complete this part myself.
Solution taken from: https://github.com/hiimjasmine00/advent-of-code/blob/master/2025/day10/part2.js
*/

function day10Two(fileInput) {
  const input = _readInput(fileInput);
  ...
  return init().then(async ({ Context }) => {
    ...
    for (const line of input) {
      ...
      for (let i = 0; i < buttons.length; i++) {
        const v = Int.const(String.fromCodePoint(i + 97));
        solver.add(v.ge(0));
        variables.push(v);
      }
      ...
      for (let i = 0; i < requirement.length; i++) {
        ...
        solver.add(condition.eq(Int.val(requirement[i])));
      }
...
//const fileInput = './2025/day10/example.txt'; // 7 - 33
const fileInput = './2025/day10/input.txt'; // 545 - 22430

console.log(day10(getMachines(fileInput)));
day10Two(fileInput).then((r) => console.log(r));
