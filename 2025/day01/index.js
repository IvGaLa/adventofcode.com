...
const day01 = (fileInput) => {
  ...
  for (const line of data) {
    const steps = (line[0] === 'L' ? -1 : 1) * parseInt(line.slice(1));
...
const day01Two = (fileInput) => {
  ...
  for (const line of data) {
    ...
    count +=
      steps > 0
        ? Math.floor((position + steps) / 100) - Math.floor(position / 100)
        : Math.floor((position - 1) / 100) -
          Math.floor((position - 1 + steps) / 100);
...
//const fileInput = './2025/day01/example.txt';
const fileInput = './2025/day01/input.txt';
